 ...
 * 古诗词常见地点坐标映射（移动端）
 *
 * 移植自桌面端 src/utils/poetry-location.ts，零运行时依赖，
 * 主包/分包均可安全引用。
 *
 * 用途：将诗词中的地点文本（如"扬州"、"瓜洲"）转换为经纬度坐标，
 * 供地图组件渲染标记使用。
 */
...
/**
 * 根据地点文本解析坐标
 * 支持「城市/景点」格式，如"扬州/瓜洲"，取第一个能解析出坐标的部分。
 *
 * @example
 *   parseLocation('扬州')        => { lng: 119.4, lat: 32.4, name: '扬州', ... }
 *   parseLocation('姑苏')        => { lng: 120.6, lat: 31.3, name: '苏州', ... }（别名）
 *   parseLocation('扬州/瓜洲')   => 扬州坐标
 *   parseLocation('未知地点')    => null
 */
export function parseLocation(locationText?: string | null): LocationCoord | null {
...
/**
 * 为文章对象批量补全 geo 字段
 * 如果 location 解析不出来，会尝试用 title 兜底（保留桌面端行为）
 */
export function enrichArticleGeo<T extends { location?: string; title?: string; geo?: LocationCoord }>(
  article: T,
): T & { geo?: LocationCoord } {
  ...
  if (coord) {
    return { ...article, geo: coord }
  }
  return article
}

/**
 * 获取所有可用地点列表（用于地图概览、提示等）
 */
